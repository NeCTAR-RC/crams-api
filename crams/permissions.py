...
from crams.settings import CRAMS_PROVISIONER_ROLE, TOKEN_EXPIRY_TIME_MINUTES
...
from crams.dateUtils import get_current_time_for_app_tz, get_minutes_elapsed
...
class IsCramsAuthenticated(permissions.IsAuthenticated):
    ...
    def has_permission(self, request, view):
        ...
        if super().has_permission(request, view):
            ...
            elapsed_mins = get_minutes_elapsed(
                get_current_time_for_app_tz(), crams_token.created)
            if elapsed_mins >= TOKEN_EXPIRY_TIME_MINUTES:
